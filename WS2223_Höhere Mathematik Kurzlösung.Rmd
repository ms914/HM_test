...
# Display stationary points and Hessian
pprint(stationary_points)
pprint(hessian)
...
x_vals = np.linspace(0.1, 2, 400)  # Avoid zero since ln(0) is undefined
...
stationary_points_numeric = [(1, -1), (1, 1), (np.exp(-1), 0)]
for point in stationary_points_numeric:
    plt.plot(point[0], point[1], 'ro')
    plt.text(point[0] + 0.02, point[1] + 0.02, f'({point[0]:.2f}, {point[1]:.2f})', color='white', fontsize=12)

plt.title(r'Contour Plot of $f(x, y) = (y^2 - x)\ln(x)$ with Stationary Points')
plt.xlabel('x')
plt.ylabel('y')
plt.grid(True)
plt.show()
